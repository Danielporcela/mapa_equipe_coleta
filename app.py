...
def remover_disponivel(cursor, codigo):
    if not codigo:
        return
    cursor.execute("DELETE FROM disponiveis WHERE codigo=?", (codigo,))
...
def adicionar_disponivel(cursor, codigo, tipo):
    ...
    if existente:
        ...
        cursor.execute("""
            INSERT INTO disponiveis (codigo, tipo, status)
            VALUES (?, ?, 'Reserva')
        """, (codigo, tipo))
...
def localizar():
    try:
        ...
        equipe = cursor.execute("""
            ...
            WHERE Motorista=? OR C1=? OR C2=? OR C3=?
        """, (codigo, codigo, codigo, codigo)).fetchone()

        if equipe:
            flash(f"📍 {codigo} está no setor {equipe['Setor_ID']}")
            conn.close()
            return redirect(url_for("equipes"))

        disponivel = cursor.execute("""
            SELECT status FROM disponiveis WHERE codigo=?
        """, (codigo,)).fetchone()

        if disponivel:
            flash(f"📍 {codigo} está em {disponivel['status']}")
            conn.close()
            return redirect(url_for("equipes"))
...
def ver_mapa(setor):
    try:
        ...
        for ext in extensoes:
            caminho = os.path.join("static", "mapas", f"{setor}{ext}")

            if os.path.exists(caminho):
                arquivo = f"{setor}{ext}"
                return render_template("mapa.html", arquivo=arquivo)

        flash(f"⚠️ Mapa do setor {setor} não encontrado")
        return redirect(url_for("equipes"))
...
def add():
    try:
        ...
        if motorista and not codigo_valido(motorista, "MOTORISTA"):
            flash("❌ Motorista inválido! Use M123")
            return redirect(url_for("equipes"))

        for c in [c1, c2, c3]:
            if c and not codigo_valido(c, "COLETOR"):
                flash("❌ Coletor inválido! Use C123")
                return redirect(url_for("equipes"))

        if not motorista and not c1 and not c2 and not c3:
            flash("❌ Informe pelo menos um motorista ou coletor!")
            return redirect(url_for("equipes"))
        ...
        for codigo in lista:
            ...
            if conflito:
                flash(f"❌ Código {codigo} já está na equipe {conflito['Setor_ID']}")
                conn.close()
                return redirect(url_for("equipes"))
...
def salvar():
    try:
        ...
        if not motorista and not c1 and not c2 and not c3:
            flash("❌ A equipe não pode ficar vazia!")
            return redirect(url_for("equipes"))
        ...
        equipe = cursor.execute("SELECT * FROM equipes WHERE id=?", (id,)).fetchone()

        antigos = [equipe["Motorista"], equipe["C1"], equipe["C2"], equipe["C3"]]
        novos = [motorista, c1, c2, c3]

        for i, antigo in enumerate(antigos):
            if antigo and antigo not in novos:
                tipo = "MOTORISTA" if i == 0 else "COLETOR"
                adicionar_disponivel(cursor, antigo, tipo)
        ...
        for codigo in lista:
            conflito = cursor.execute("""
                ...
                WHERE (Motorista=? OR C1=? OR C2=? OR C3=?)
                AND id != ?
            """, (codigo, codigo, codigo, codigo, id)).fetchone()

            if conflito:
                flash("❌ Código já está em outra equipe")
                conn.close()
                return redirect(url_for("equipes"))
...
def excluir(id):
    ...
    if equipe:
        ...
        for tipo, codigo in pessoas:
            adicionar_disponivel(cursor, codigo, tipo)

    cursor.execute("DELETE FROM equipes WHERE id=?", (id,))
    conn.commit()
    conn.close()
    return redirect(url_for("equipes"))
...
def converter_todos():
    try:
        ...
        equipes = cursor.execute("SELECT id, Setor_ID FROM equipes").fetchall()

        alterados = 0

        for e in equipes:
            atual = (e["Setor_ID"] or "").strip().upper()

            novo = mapa.get(atual) if modo == "ID" else mapa_inverso.get(atual)

            if novo and atual != novo:
                cursor.execute(
                    "UPDATE equipes SET Setor_ID=? WHERE id=?",
                    (novo, e["id"])
                )
                alterados += 1
        ...
        flash(f"✅ {alterados} setores convertidos para {modo}")
...
def atualizar_status():
    ...
    conn = conectar()
    conn.execute("UPDATE disponiveis SET status=? WHERE codigo=?",
                 (data["status"], data["codigo"]))
    conn.commit()
    conn.close()
...
def excluir_disponivel():
    data = request.get_json()
    conn = conectar()
    conn.execute("DELETE FROM disponiveis WHERE codigo=?",
                 (data["codigo"],))
    conn.commit()
    conn.close()
    return jsonify(ok=True)
...
def cadastrar_status():
    try:
        codigo = normalizar(request.form.get("codigo"))
